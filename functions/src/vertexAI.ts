...
// 政策課題生成
exports.createPolicyIssue = async function (
...
`
  ...
  try {
    ...
    return {
      ...
      options: policyData.options.map((opt: PolicyOption, index: number) => ({
...
// AI官僚メッセージ生成
exports.createBureaucratMessage = async function (
  countryData: CountryData,
...
// 市民の声生成
exports.createCitizenVoices = async function (
  policyIssue: PolicyIssue,
  selectedOption: PolicyOption,
  reasoning: string,
  countryData: CountryData,
...
`
  ...
  try {
    ...
    // CitizenVoice形式に変換
    const citizenVoices = voicesData.voices.map(
      (voice: CitizenVoice, index: number) => ({
